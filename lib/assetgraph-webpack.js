...
module.exports = async function bundleWebpack(assetGraph, { configPath } = {}) {
  let webpack;
  try {
    ...
    if (configPath) {
      throw new Error(
        `Webpack config path given (${configPath}), but webpack itself could not be found. Please install it in your project: ${e.stack}`
      );
    }
  }
  ...
  if (!hasHtmlWebpackPlugin) {
    ...
    if (!seenReferencedEntryPoints) {
      assetGraph.info(new Error('No matching webpack bundles found, skipping'));
      return;
    }
  }
  ...
  for (const asset of statsObj.assets) {
    ...
    if (existingAsset) {
      assetGraph.info(new Error(`Replacing previously built artifact: ${url}`));
  ...
  if (existingAssetsWereReplaced) {
    assetGraph.info(
      new Error(
        `Please remove ${config.output.path} before building with assetgraph to speed up the process`
      )
    );
  }
        ...
        if (
          ...
          if (
            ...
            lastStatement.argument.callee.object.arguments[0].object.name ===
              'ids' &&
            lastStatement.argument.callee.object.arguments[0].property.type ===
              'Literal'
          ) {
